...
projectOption(canon.command('add')
  ...
  .action(async (title, opts) => {
    printPath(await createCanon(path.resolve(opts.project), title, opts.content, {
      strength: opts.strength,
      source: opts.source
    }))
  })
projectOption(canon.command('import')
  ...
  .action(async (file, opts) => {
    printPath(await importCanonFile(path.resolve(opts.project), path.resolve(file), {
      strength: opts.strength,
      source: opts.source
    }))
  })
projectOption(canon.command('list').description('List canon items'))
  .action(async (opts) => {
    await printDocs(path.resolve(opts.project), 'canon')
  })

const character = program.command('character').description('Manage characters')
projectOption(character.command('add')
  ...
  .action(async (name, opts) => {
    printPath(await createCharacter(path.resolve(opts.project), name, {
      role: opts.role,
...
projectOption(character.command('list').description('List characters'))
  .action(async (opts) => {
...
projectOption(timeline.command('append')
  ...
  .action(async (title, opts) => {
    printPath(await appendTimelineEvent(path.resolve(opts.project), title, {
      date: opts.date,
...
projectOption(timeline.command('list').description('List timeline events'))
  .action(async (opts) => {
    await printDocs(path.resolve(opts.project), 'timeline_event')
  })
projectOption(timeline.command('check').description('Check timeline links'))
  .action(async (opts) => {
    ...
    for (const doc of docs) {
      if (doc.data.previous && !ids.has(doc.data.previous)) console.log(`[error] ${doc.data.id}: missing previous ${doc.data.previous}`)
      if (doc.data.next && !ids.has(doc.data.next)) console.log(`[error] ${doc.data.id}: missing next ${doc.data.next}`)
    }
...
projectOption(route.command('add')
  ...
  .action(async (opts) => {
    printPath(await createRoute(path.resolve(opts.project), opts.from, opts.to, {
...
projectOption(outline.command('add')
  ...
  .action(async (level: OutlineDoc['level'], title, opts) => {
    printPath(await createOutline(path.resolve(opts.project), level, title, {
...
projectOption(scene.command('create')
  ...
  .action(async (title, opts) => {
    printPath(await createScene(path.resolve(opts.project), title, {
...
projectOption(scene.command('list').description('List scenes'))
  .action(async (opts) => {
    await printDocs(path.resolve(opts.project), 'scene')
  })

projectOption(program.command('index').description('Build project index'))
  .action(async (opts) => {
    const index = await buildIndex(path.resolve(opts.project))
    console.log(`Indexed ${index.entries.length} documents.`)
  })

projectOption(program.command('context')
...
projectOption(program.command('generate')
  .argument('<scene-id>', 'Scene id')
...
projectOption(program.command('check')
  .argument('<scene-id>', 'Scene id')
  ...
  .action(async (sceneId, opts) => {
    ...
    if (opts.run) {
      ...
      }, 'check-report.md', formatted)
    }
    console.log(formatted)
  })

const run = program.command('run').description('Manage generation runs')
projectOption(run.command('list').description('List run directories'))
  .action(async (opts) => {
    const docs = await listDocs(path.resolve(opts.project), 'scene')
    console.log(`Scenes: ${docs.length}`)
    console.log('Run listing is file-system based; inspect runs/ for now.')
  })
projectOption(run.command('show')
...
projectOption(run.command('accept')
  .argument('<run-id>', 'Run id')
...
async function printDocs(projectRoot: string, type: Parameters<typeof listDocs>[1]) {
  ...
  for (const doc of docs) {
    console.log(`${doc.data.id}\t${doc.data.title}\t${doc.data.status}\t${path.relative(projectRoot, doc.path)}`)
  }
}
